...
void Buffer::update_mask_buffer(int rank_to_mask, bool mask) {
    ...
    if (!mask) {
        if (scale_stage_pending) {
            EP_HOST_ASSERT(
                std::find(staged_ranks.begin(), staged_ranks.end(), rank_to_mask)
                    != staged_ranks.end()
                && "cannot unmask a rank outside the pending scale stage");
            _publish_staged_slot_locked();
        }
...
void Buffer::_nixl_agent_init() {
    ...
    const char* num_workers_env = std::getenv("NIXL_EP_NUM_UCX_WORKERS");
    init_params["num_workers"] = num_workers_env ? num_workers_env : "1";
